...
const runtimeStore = read('ios/BideRuntime/CodeRuntimeStore.swift');
...
assert.match(project, /- path: BideRuntime/);
...
assert.match(workspace, /Runs locally on this device/);
...
for (const coverage of [
  ...
  'lm(y ~ x',
  'saved_mean',
  'session state did not persist',
]) {
  assert.ok(runtimeSmoke.includes(coverage), `General runtime coverage missing: ${coverage}`);
}
...
console.log('bIDE iOS isolated Python/R runtime source guard passed.');
