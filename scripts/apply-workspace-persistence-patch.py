...
BRANCH_NOTE = "workspace persistence patch"

ide = Path("src/pages/IDE.tsx")
text = ide.read_text()

old_hook = "  const { saveFile, loadFiles, deleteFile, isReady: dbReady } = useIndexedDB();"
new_hook = "  const { saveFile, loadFiles, deleteFile, replaceFiles, isReady: dbReady } = useIndexedDB();"
if old_hook not in text:
    if new_hook not in text:
        raise SystemExit("useIndexedDB destructure drifted; refusing partial patch")
else:
    text = text.replace(old_hook, new_hook, 1)

start_marker = "      <WorkspaceManager\n        open={workspaceManagerOpen}"
start = text.find(start_marker)
if start < 0:
    raise SystemExit("WorkspaceManager JSX start drifted; refusing partial patch")
end = text.find("      />", start)
if end < 0:
    raise SystemExit("WorkspaceManager JSX end drifted; refusing partial patch")
end += len("      />")

new_block = """      <WorkspaceManager
        open={workspaceManagerOpen}
...
for old, new in replacements:
    if old in t:
        if t.count(old) != 1:
            raise SystemExit(f"Workspace generated type block drifted: expected one match, got {t.count(old)}")
        t = t.replace(old, new, 1)
    elif new not in t:
        raise SystemExit("Workspace generated type block drifted; refusing partial patch")

types.write_text(t)
print(f"Applied {BRANCH_NOTE}")
